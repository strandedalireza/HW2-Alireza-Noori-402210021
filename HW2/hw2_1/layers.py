...
class FCLayer(nn.Module):
    ...
    def backward(self, delta):
        ...
        """
        delta_next = torch.matmul(delta, self.W.T)
        dW = torch.matmul(self.X.T, delta)
        db = torch.sum(delta, dim=0, keepdim=True)
...
class ReLULayer(nn.Module):
    ...
    def forward(self, X):
        ...
        """
        self.X = X
        out = torch.maximum(X, torch.tensor(0.0).to(X.device))
        return out
